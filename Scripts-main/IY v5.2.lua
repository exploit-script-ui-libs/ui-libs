...
addcmd('f3x',{'fex'},function(args, speaker)
    notify('Third Party Blocker','This function is using third party script')
end)
...
addcmd('console',{},function(args, speaker)
	...
	local s, e = loadstring([==[local a=Instance.new;local b=UDim2.new;local c=Color3.new;local d=math.max;local tick=tick;local pairs=pairs;local e=os.time;local f=false;if syn then require=getrenv().require end;local g="Game"local h="DeveloperConsole_InitialOpenTab"local i="DeveloperConsole_ClickToOpenOpenTab"local j=game:GetService("CoreGui")local k=j:FindFirstChild("RobloxGui")local l=k:FindFirstChild("Modules")local m=game:GetService("ContextActionService")local n=game:GetService("TextService")local o=game:GetService("GuiService")local p=game:GetService("VRService")local q=o:IsTenFootInterface()local r=require(j:WaitForChild("RobloxGui"):WaitForChild("Modules"):WaitForChild("Stats"):WaitForChild("ClientMemoryAnalyzer"))local s=require(j:WaitForChild("RobloxGui"):WaitForChild("Modules"):WaitForChild("Stats"):WaitForChild("ServerMemoryAnalyzer"))local t=require(j:WaitForChild("RobloxGui"):WaitForChild("Modules"):WaitForChild("Stats"):WaitForChild("StatsUtils"))local u;do local v=Color3.fromRGB;local w=Color3.new(0.1,0.1,0.1)local x=Color3.new(1,1,1)local y=Color3.new(1,1,1)pcall(function()local z=game:GetService("Players")if not z or not z.LocalPlayer then return end;local A={[56449]={v(255,63,127)},[6949935]={v(255,63,127)}}local B=A[z.LocalPlayer.UserId]if B then w=B[1]or w;x=B[2]or x end end)u={ZINDEX=6,Font=Enum.Font.SourceSans,FontBold=Enum.Font.SourceSansBold,HandleHeight=24,TabHeight=28,GearSize=24,BorderSize=2,CommandLineHeight=22,OptionAreaHeight=56,FrameColor=w,FrameTransparency=0.5,OptionsFrameColor=y,TextColor=x,MessageColors={[0]=Color3.new(1,1,1),[1]=Color3.new(0.4,0.5,1),[2]=Color3.new(1,0.6,0.4),[3]=Color3.new(1,0,0)},ScrollbarFrameColor=w,ScrollbarBarColor=w,ScriptButtonHeight=32,ScriptButtonColor=Color3.new(0,1/3,2/3),ScriptButtonTransparency=0.5,CheckboxSize=24,ChartTitleHeight=20,ChartGraphHeight=64,ChartDataHeight=24,ChartHeight=0,ChartWidth=620,ChartGraphDirection=1,GetButtonDownColor=function(C)local D,E,F=C.r,C.g,C.b;return Color3.new(1-0.75*(1-D),1-0.75*(1-E),1-0.75*(1-F))end,GetButtonHoverColor=function(C)local D,E,F=C.r,C.g,C.b;return Color3.new(1-0.875*(1-D),1-0.875*(1-E),1-0.875*(1-F))end}u.ChartHeight=u.ChartTitleHeight+u.ChartGraphHeight+u.ChartDataHeight+u.BorderSize end;local G={}do local function H(I,J,K)local L=Instance.new(I,J)L.ZIndex=0;if K then L.Name=K end;return L end;local M=UDim2.new(1,0,1,0)local function N(L)L.BackgroundColor3=u.FrameColor;L.BackgroundTransparency=u.FrameTransparency;L.BorderSizePixel=0 end;local function O(L,P)L.Font=u.Font;L.TextColor3=u.TextColor;L.Text=P or L.Text end;function G.Frame(J,K)local L=H('Frame',J,K)N(L)return L end;function G.TextLabel(J,K,P)local L=H('TextLabel',J,K)N(L)O(L,P)return L end;function G.TextBox(J,K,P)local L=H('TextBox',J,K)N(L)O(L,P)return L end;function G.TextButton(J,K,P)local L=H('TextButton',J,K)N(L)O(L,P)return L end;function G.Button(J,K)local L=H('TextButton',J,K)N(L)L.Text=""return L end;function G.ImageButton(J,K,Q)local L=H('ImageButton',J,K)N(L)L.Image=Q or""L.Size=M;return L end;function G.FolderFrame(J,K)local L=H('Frame',J,K)L.BackgroundTransparency=1;L.Size=M;return L end;function G.InvisibleTextLabel(J,K,P)local L=H('TextLabel',J,K)O(L,P)L.BackgroundTransparency=1;return L end;function G.InvisibleButton(J,K,P)local L=H('TextButton',J,K)L.BackgroundTransparency=1;L.Text=""return L end;function G.InvisibleImageLabel(J,K,Q)local L=H('ImageLabel',J,K)L.BackgroundTransparency=1;L.Image=Q or""L.Size=M;return L end end;local function R(S)local T,U=pcall(function()return settings():GetFFlag(S)end)return T and U end;local V=true;local W=-1;local X=14;local Y=8;local function Z()local _={}local a0=Instance.new('BindableEvent')local a1={}function _:connect(a2)if self~=_ then error("connect must be called with `:`, not `.`",2)end;if type(a2)~='function'then error("Argument #1 of connect must be a function, got a "..type(a2),2)end;local a3=a0.Event:Connect(a2)a1[a3]=true;local a4={}function a4:disconnect()a3:Disconnect()a1[a3]=nil end;a4.Disconnect=a4.disconnect;return a4 end;function _:disconnect()if self~=_ then error("disconnect must be called with `:`, not `.`",2)end;for a3,a5 in pairs(a1)do a3:Disconnect()a1[a3]=nil end end;function _:wait()if self~=_ then error("wait must be called with `:`, not `.`",2)end;return a0.Event:Wait()end;function _:fire(...)if self~=_ then error("fire must be called with `:`, not `.`",2)end;a0:Fire(...)end;_.Connect=_.connect;_.Disconnect=_.disconnect;_.Wait=_.wait;_.Fire=_.fire;return _ end;local a6;do local a7={}local a8={__index=a7}function a7.fire(_,...)return _.Signal:fire(...)end;function a7.wait(_,...)return _.Signal:wait(...)end;function a7.connect(_,a2)local a9=type(a2)if a9=='table'or a9=='userdata'then local aa=a2;function a2()aa:disconnect()end elseif a9~='function'then error('Invalid disconnect method type: '..a9,2)end;local aa;aa=_.Signal:connect(function(...)if aa then aa:disconnect()aa=nil;a2(...)end end)return aa end;function a6()return setmetatable({Signal=Z()},a8)end end;local ab=game:GetService('UserInputService')local ac=game:GetService('RunService')local ad=ab.TouchEnabled;local ae={}local a7={}local a8={__index=a7}function a7.ConnectSetVisible(af,a2)a2(af.Visible)return af.VisibleChanged:connect(function(ag)a2(ag)end)end;function a7.ConnectObjectSetVisible(af,ah,a2)a2(ah,af.Visible)return af.VisibleChanged:connect(function(ag)a2(ah,ag)end)end;local function ai(ah,aj,ak)return ah.Changed:connect(function(al)if al==aj then ak(ah[aj])end end)end;function a7.ResetFrameDimensions(af)af.Frame.Size=b(0.5,20,0.5,20)local am=af.Frame.AbsoluteSize;af:SetFrameSize(am.x,am.y)local an=af.Frame.Size;af.Frame.Position=b(0.5,-an.X.Offset/2,0.5,-an.Y.Offset/2)end;function a7.BoundFrameSize(af,ao,ap)return d(ao,400),d(ap,200)end;function a7.SetFrameSize(af,ao,ap)ao,ap=af:BoundFrameSize(ao,ap)af.Frame.Size=b(0,ao,0,ap)end;function a7.BoundFramePosition(af,ao,ap)return ao,d(ap,0)end;function a7.SetFramePosition(af,ao,ap)ao,ap=af:BoundFramePosition(ao,ap)af.Frame.Position=b(0,ao,0,ap)end;function a7.SetVisible(af,ag,aq)if af.Visible==ag then return end;af.Visible=ag;af.VisibleChanged:fire(ag)if af.Frame then af.Frame.Visible=ag end;if ag then af:ResetFrameDimensions()local ar=af:GetCurrentOpenTab()if ar~=nil then ar:RecordInitialOpen()end end;if p.VREnabled then if ag then ab.OverrideMouseIconBehavior=Enum.OverrideMouseIconBehavior.ForceShow else ab.OverrideMouseIconBehavior=Enum.OverrideMouseIconBehavior.ForceHide end end end;function ae.new(as,at,au)local av=Z()local af={ScreenGui=as,Permissions=at,MessagesAndStats=au,Initialized=false,Visible=false,Tabs={},CurrentOpenedTab=nil,VisibleChanged=av}setmetatable(af,a8)af:EnableGUIMouse()local aw=G.Button(as,'DeveloperConsole')aw.AutoButtonColor=false;aw.Visible=af.Visible;aw.Selectable=not q;local function ax()aw.Modal=p.VREnabled end;ax()p:GetPropertyChangedSignal("VREnabled"):connect(ax)af.Frame=aw;af:ResetFrameDimensions()local ay=G.Button(aw,'Handle')ay.Size=b(1,-(u.HandleHeight+u.BorderSize),0,u.HandleHeight)ay.Selectable=not q;ay.Modal=true;ay.AutoButtonColor=false;do local az=G.InvisibleTextLabel(ay,'Title',"Developer Console")az.Size=b(1,-5,1,0)az.Position=b(0,5,0,0)az.FontSize=Enum.FontSize.Size18;az.TextXAlignment=Enum.TextXAlignment.Left end;local function aA(aB,aC)aB.Size=b(aC,0,aC,0)aB.Position=b((1-aC)/2,0,(1-aC)/2,0)end;local function aD(K,ao,ap,Q,aC)local aE=G.Button(aw,K)aE.Size=b(0,u.HandleHeight,0,u.HandleHeight)aE.Position=b(ao,-ao*u.HandleHeight,ap,-ap*u.HandleHeight)local aB=G.InvisibleImageLabel(aE,'Image',Q)aA(aB,aC)return aE,aB end;do local aF,aG=aD('Exit',1,0,'https://www.roblox.com/asset/?id=261878266',2/3)aF.AutoButtonColor=false;aF.Visible=not q;aF.Selectable=not q;local aH=af:CreateButtonEffectFunction(aF)af:ConnectButtonHover(aF,function(aI,aJ)if aJ and not aI then aA(aG,3/4)else aA(aG,2/3)end;aH(aI,aJ)end)aF.MouseButton1Click:connect(function()af:SetVisible(false,true)end)end;do do local aK,aL=aD('Resize',1,1,'https://www.roblox.com/asset/?id=261880743',1)aL.Position=b(0,0,0,0)aL.Size=b(1,0,1,0)aK.Selectable=not q;local aM=false;local aH=af:CreateButtonEffectFunction(aK)af:ConnectButtonDragging(aK,function()local aN,aO=aw.AbsoluteSize.X,aw.AbsoluteSize.Y;return function(aP,aQ)af:SetFrameSize(aN+aP,aO+aQ)end end,function(aI,aJ)aM=aI;aH(aI,aJ)end)end;do local aw=af.Frame;local ay=aw.Handle;local aH=af:CreateButtonEffectFunction(ay)af:ConnectButtonDragging(ay,function()local ao,ap=aw.AbsolutePosition.X,aw.AbsolutePosition.Y;return function(aP,aQ)af:SetFramePosition(ao+aP,ap+aQ)end end,aH)end end;local aR=G.FolderFrame(aw,'Interior')aR.Position=b(0,0,0,u.HandleHeight)aR.Size=b(1,-(u.HandleHeight+u.BorderSize*2),1,-(u.HandleHeight+u.BorderSize))local aS=G.FolderFrame(aR,'WindowContainer')aS.Size=b(1,0,1,-u.TabHeight)aS.Position=b(0,u.BorderSize,0,u.TabHeight)local aT=G.Frame(aS,'Window')aT.Size=b(1,0,1,0)aT.Position=b(0,0,0,0)aT.ClipsDescendants=true;local aU=G.FolderFrame(aT,'Body')do local aV=af:CreateScrollbar()af.WindowScrollbar=aV;local aW=aV.Frame;aW.Parent=aw;aW.Size=b(0,u.HandleHeight,1,-(u.HandleHeight+u.BorderSize)*2)aW.Position=b(1,-u.HandleHeight,0,u.HandleHeight+u.BorderSize)af:ApplyScrollbarToFrame(aV,aT,aU,aw)end;local aX=G.FolderFrame(aR,'Tabs')aX.Size=b(1,-(u.GearSize+u.BorderSize),0,u.TabHeight)aX.Position=b(0,0,0,0)aX.ClipsDescendants=true;local aY=G.InvisibleButton(aw,'OptionsButton')local aZ=G.FolderFrame(aR,'OptionsClippingFrame')aZ.ClipsDescendants=true;aZ.Position=b(0,0,0,0)aZ.Size=b(1,0,0,0)local a_=G.FolderFrame(aZ,'OptionsFrame')a_.Size=b(1,0,0,u.OptionAreaHeight)a_.Position=b(0,0,0,u.OptionAreaHeight)do local b0=u.GearSize;local b1=u.TabHeight;local b2=(b1-b0)/2;aY.Size=b(0,u.GearSize,0,u.GearSize)aY.Position=b(1,-(u.GearSize+b2+u.HandleHeight),0,u.HandleHeight+b2)local b3=G.InvisibleImageLabel(aY,'Image','https://www.roblox.com/asset/?id=261882463')local b4=af:GenerateOptionButtonAnimationToggle(aR,aY,b3,aX,aZ,a_)local b5=false;aY.MouseButton1Click:connect(function()b5=not b5;b4(b5)end)end;local b6;local b7,b8;local b9,ba;local bb;local bc,bd;do local function be(bf,ak)local _={Value=true}local aw=G.FolderFrame(nil,'Checkbox')_.Frame=aw;aw.Size=b(0,u.CheckboxSize,0,u.CheckboxSize)aw.BackgroundColor3=bf;local bg=2;local function bh(bi,bj,bk)local bl=1-bi;local bh=G.Frame(aw,'Border')bh.BackgroundColor3=bf;bh.BackgroundTransparency=0;bh.Size=b(bi,bl*bg,bl,bi*bg)bh.Position=b(bj,-bj*bg,bk,-bk*bg)end;bh(1,0,0)bh(1,0,1)bh(0,0,0)bh(0,1,0)local aE=G.Button(aw,'Button')aE.Size=b(1,-bg*2,1,-bg*2)aE.Position=b(0,bg,0,bg)local aH=af:CreateButtonEffectFunction(aE)local bm=G.Frame(aE,'Check')local bg=4;bm.Size=b(1,-bg*2,1,-bg*2)bm.Position=b(0,bg,0,bg)bm.BackgroundColor3=bf;bm.BackgroundTransparency=0;af:ConnectButtonHover(aE,aH)function _.SetValue(_,bn)if bn==_.Value then return end;_.Value=bn;bm.Visible=bn;_.Value=bn;ak(bn)end;aE.MouseButton1Click:connect(function()_:SetValue(not _.Value)end)return _ end;local bo=string.find;local bp;function b7(P)return not bp or bo(P:lower(),bp)end;local bq={}function bb(br)return bq[br.Type]and(not bp or bo(br.Message:lower(),bp))end;b9=Z()ba=Z()bc=Z()bd=Z()local bs={}function b6(bt)for bu,bv in pairs(bs)do bv.Visible=bt[bu]or false end end;do local bv=G.FolderFrame(a_,'Log')bv.Visible=false;bs.Log=bv;local bw=G.InvisibleTextLabel(bv,'FilterLabel',"Filters")bw.FontSize='Size18'bw.TextXAlignment='Left'bw.Size=b(0,54,0,u.CheckboxSize)bw.Position=b(0,4,0,2)do local ao=bw.Size.X.Offset;local bx=u.MessageColors;for by=0,#bx do local bz=be(bx[by],function(bn)bq[by]=bn;bc:fire()end)bq[by]=bz.Value;bz.Frame.Parent=bv;bz.Frame.Position=b(0,ao,0,4)ao=ao+u.CheckboxSize+4 end;do ao=ao+8;local bw=G.InvisibleTextLabel(bv,'WrapLabel',"Word Wrap")bw.FontSize='Size18'bw.TextXAlignment='Left'bw.Size=b(0,54+u.CheckboxSize,0,u.CheckboxSize)bw.Position=b(0,ao+4,0,2)local bz=be(Color3.new(0.65,0.65,0.65),function(bn)bd:fire(bn)end)bz:SetValue(false)bz.Frame.Parent=bv;bz.Frame.Position=b(0,ao+bw.Size.X.Offset,0,4)end end end;do local bv=G.FolderFrame(a_,'Stats')bv.Visible=false;bs.Scripts=bv;do local ao=0;do ao=ao+4;local bw=G.InvisibleTextLabel(bv,'FilterLabel',"Show inactive")bw.FontSize='Size18'bw.TextXAlignment='Left'bw.Size=b(0,bw.TextBounds.X+6,0,u.CheckboxSize)bw.Position=b(0,ao,0,2)ao=ao+bw.Size.X.Offset;local bA;local function bB(bC)local bD=bC.Stats;if bD then local bE=bD[#bD]if bE then return bE[1]>0.000001 or bE[2]>0.000001 end end;return false end;function b8(bC)return(bA or bB(bC))and(not bp or bo(bC.Name:lower(),bp))end;local bz=be(c(1,1,1),function(bn)bA=bn;ba:fire()end)bA=bz.Value;bz.Frame.Parent=bv;bz.Frame.Position=b(0,ao,0,4)ao=ao+u.CheckboxSize+4 end;ao=ao+8 end end;do local bv=G.FolderFrame(a_,'Search')bv.Visible=false;bs.Search=bv;local bw=G.InvisibleTextLabel(bv,'FilterLabel',"Contains:")bw.FontSize='Size18'bw.TextXAlignment='Left'bw.Size=b(0,60,0,u.CheckboxSize)bw.Position=b(0,4,0,4+u.CheckboxSize+4)local bF=G.TextBox(bv,'ContainsFilter')bF.ClearTextOnFocus=true;bF.FontSize='Size18'bF.TextXAlignment='Left'bF.Size=b(0,150,0,u.CheckboxSize)bF.Position=b(0,bw.Position.X.Offset+bw.Size.X.Offset+4,0,4+u.CheckboxSize+4)bF.Text=""local bG=Color3.new(0,0.5,0)local C=bF.BackgroundColor3;ai(bF,'Text',function(P)P=P:lower()if P==""then P=nil end;if P==bp then return end;bF.BackgroundColor3=P and bG or C;bp=P;bc:fire()b9:fire()end)ai(bF,'TextBounds',function(bH)bF.Size=b(0,math.max(bH.X,150),0,u.CheckboxSize)end)end end;do local function bI(K,P,bJ,bK,bL,bM)local bN=G.FolderFrame(aU,K)local bO,bP;local bQ=a6()local ar=af:AddTab(P,bN,function(b5)if bP then bP.Frame.Visible=b5 end;if b5 then b6({Log=true,Search=true})if not bO then bO=af:CreateOutput(bJ:GetMessages(),bb)bO.Frame.Parent=bN end;bO:SetVisible(true)if bK then if b5 and not bP then bP=af:CreateCommandLine()bP.Frame.Parent=aw;bP.Frame.Size=b(1,-(u.HandleHeight+u.BorderSize*2),0,u.CommandLineHeight)bP.Frame.Position=b(0,u.BorderSize,1,-(u.CommandLineHeight+u.BorderSize))bP.CommandInputted:connect(bL)end end;aT.Size=bK and b(1,0,1,-u.HandleHeight)or b(1,0,1,0)local bR=bJ:GetMessages()local bS=bO:RefreshMessages()aU.Size=b(1,0,0,bS)bQ:connect(bO.HeightChanged:connect(function(bS)aU.Size=b(1,0,0,bS)end))aU.Size=b(1,0,0,bO.Height)bQ:connect(bJ.MessageAdded:connect(function(br)bO:RefreshMessages(#bR)end))bQ:connect(bc:connect(function()bO:RefreshMessages()end))bQ:connect(bd:connect(function(bT)bO:SetTextWrappedEnabled(bT)end))else if bO then bO:SetVisible(false)end;aT.Size=b(1,0,1,0)bQ:fire()end;if bM then bM(b5)end end)return ar end;if at.MayViewClientLog then local ar=bI('ClientLog',"Client Log",af.MessagesAndStats.OutputMessageSyncLocal,true,function(P)if#P<=1 then return end;local bh,bU=loadstring(P)if bU then warn(bU)return end;bh()end)ar:SetVisible(true)ar:SetOpen(true)end end;return af end;do function a7.EnableGUIMouse(af)local bw=Instance.new("ImageLabel")bw.BackgroundTransparency=1;bw.BorderSizePixel=0;bw.Size=UDim2.new(0,64,0,64)bw.Image="rbxasset://Textures/ArrowFarCursor.png"bw.Name="BackupMouse"bw.ZIndex=u.ZINDEX+2;local bQ=a6()local bT=false;local bV=game:GetService("Players").LocalPlayer:GetMouse()local function bW()local bX=af.Visible and not ab.MouseIconEnabled;if bX==bT then return end;bT=bX;bw.Visible=bT;bw.Parent=bT and af.ScreenGui or nil;bQ:fire()if bT then bw.Position=UDim2.new(0,bV.X-32,0,bV.Y-32)bQ:connect(ab.InputChanged:connect(function(bY)if bY.UserInputType==Enum.UserInputType.MouseMovement then bw.Position=UDim2.new(0,bV.X-32,0,bV.Y-32)end end))end end;bW()local bZ;af.VisibleChanged:connect(function(ag)if bZ then bZ:disconnect()bZ=nil end;bZ=ab.Changed:connect(bW)bW()end)end end;do function a7.CreateCommandLine(af)local _={CommandInputted=Z()}local aw=G.FolderFrame(nil,'CommandLine')_.Frame=aw;aw.Size=b(1,0,0,u.CommandLineHeight)local b_=G.Frame(aw,'TextBoxFrame')b_.Size=b(1,0,0,u.CommandLineHeight)b_.Position=b(0,0,0,0)b_.ClipsDescendants=true;local bw=G.InvisibleTextLabel(b_,'Label',">")bw.Position=b(0,4,0,0)bw.Size=b(0,12,1,-1)bw.FontSize='Size14'local c0="Type command here"local bF=G.TextBox(b_,'TextBox')bF.BackgroundTransparency=1;bF.Text=c0;bF.ClearTextOnFocus=false;local bg=2;bF.Size=b(1,-(bg*2)-4-12,0,500)bF.Position=b(0,4+12+bg,0,0)bF.TextXAlignment='Left'bF.TextYAlignment='Top'bF.FontSize='Size18'bF.TextWrapped=true;if q then local c1=Instance.new('ImageLabel')c1.Name="SelectionImage"c1.Size=UDim2.new(1,b_.AbsoluteSize.x+36,0,u.CommandLineHeight+24)c1.Position=UDim2.new(0,-18,0,-12)c1.Image='rbxasset://textures/ui/SelectionBox.png'c1.ScaleType=Enum.ScaleType.Slice;c1.SliceCenter=Rect.new(21,21,41,41)c1.BackgroundTransparency=1;bF.SelectionImageObject=c1 end;do local c2=b(1,0,0,u.CommandLineHeight)local c3=true;bF.Changed:connect(function(aj)if aj=='TextBounds'or aj=='AbsoluteSize'then if c3 then c3=false;return end;local bH=bF.TextBounds;if bH.Y>u.CommandLineHeight then b_.Size=b(1,0,0,bH.Y+2)else b_.Size=c2 end end end)end;local bQ=a6()local c4=0;local c5={}local c6=false;local function c7(P,c8)if c6 then table.remove(c5,1)end;if c5[1]==P then c6=c6 and c8;return end;c6=c8;if not c8 then for by=#c5,1,-1 do if c5[by]==P then table.remove(c5,by)end end end;table.insert(c5,1,P)end;local function c9(ca)c4=c4+ca;if c4<1 then c4=1 elseif c4>#c5 then c4=#c5 end;if c5[c4]then bF.Text=c5[c4]end end;local cb=false;bF.Focused:connect(function()if bF.Text==c0 then bF.Text=""end;bQ:fire()c4=0;bQ:connect(ab.InputBegan:connect(function(bY)if bY.KeyCode==Enum.KeyCode.Up then if c4==0 and not cb then c7(bF.Text,true)c4=1 end;c9(1)elseif bY.KeyCode==Enum.KeyCode.Down then c9(-1)end end))end)bF.FocusLost:connect(function(cc)bQ:fire()if cc then cb=false;local P=bF.Text;c7(P,false)_.CommandInputted:fire(P)bF.Text=""if not q then bF:CaptureFocus()end else c4=0;cb=true;c7(bF.Text,true)if bF.Text==""then bF.Text=c0 end end end)return _ end end;do local bg=5;local cd=b(1,-bg,0,2048)local ce=u.MessageColors;local cf=c(0.5,0,1)local function cg(br)return false end;function a7.CreateOutput(af,bR,bb)local ch=Z()local bO={Visible=false,Height=0,HeightChanged=ch,MessagesDirty=false,MessagesDirtyPosition=1}local function ci(bS)bS=bS+4;bO.Height=bS;ch:fire(bS)end;local aw=G.FolderFrame(nil,'Output')aw.ClipsDescendants=true;bO.Frame=aw;local cj=false;do local ck=0;ai(aw,'AbsoluteSize',function(cl)local cm=cl.X;if cm~=ck then ck=cm;bO:RefreshMessages()end end)end;local cn={}local co={}local function cp()if not bO.Visible then return end;local ap=1;for by=1,#cn do local bw=cn[by]bw.TextWrapped=cj;local bS=bw.TextBounds.Y;bw.Size=cd;bw.Position=b(0,bg,0,ap)ap=ap+bS;if bS>16 then ap=ap+4 end end;ci(ap)end;local cq=2048;local function cr(cs)if not bO.Visible then return end;local ap=1;local ct=0;cs=nil;if cs then local cu;for by=cs,d(1,#bR-cq),-1 do if co[by]then cu=co[by]break end end;if cn[cu]then ct=cu;local bw=cn[cu]ap=bw.Position.Y.Offset+bw.Size.Y.Offset else cs=nil end end;for by=cs or d(1,#bR-cq),#bR do local br=bR[by]if bb(br)then ct=ct+1;co[by]=ct;local bw=cn[ct]if not bw then bw=a('TextLabel',aw)bw.ZIndex=u.ZINDEX;bw.BackgroundTransparency=1;bw.FontSize='Size10'bw.TextXAlignment='Left'bw.TextYAlignment='Top'cn[ct]=bw end;bw.TextWrapped=cj;bw.Size=cd;bw.TextColor3=ce[br.Type]or cf;bw.Text=br.Time.." -- "..br.Message;local bS=bw.TextBounds.Y;bw.Size=cd;bw.Position=b(0,bg,0,ap)ap=ap+bS;if bS>16 then ap=ap+4 end else co[by]=false end end;for by=#cn,ct+1,-1 do cn[by]:Destroy()cn[by]=nil end;ci(ap)end;function bO.SetMessagesDirty(bO,cs)if bO.MessagesDirty then return end;bO.MessagesDirty=true;bO.MessagesDirtyPosition=cs end;local cv;function bO.RefreshMessages(bO,cs)if not bO.Visible then return end;if not cv then cv=true;coroutine.wrap(function()wait()cv=false;cr()end)()end end;function bO.SetTextWrappedEnabled(bO,cw)if cw==cj then return end;cj=cw;cp()end;function bO.SetVisible(bO,ag)if ag==bO.Visible then return end;bO.Visible=ag;if ag then cr()else for by=#cn,1,-1 do cn[by]:Destroy()cn[by]=nil end end end;return bO end end;function a7.GetCurrentOpenTab(af)local cx=af.Tabs;if cx==nil then return nil end;for by=1,#cx do local ar=cx[by]if ar.Open then return ar end end;return nil end;function a7.RefreshTabs(af)local ao=u.BorderSize;local cx=af.Tabs;for by=1,#cx do local ar=cx[by]if ar.ButtonFrame.Visible then ao=ao+3;ar.ButtonFrame.Position=b(0,ao,0,0)ao=ao+ar.ButtonFrame.AbsoluteSize.X+3 end end end;function a7.AddTab(af,P,aU,bM,cy)aU.Visible=false;local ar={Open=false,Visible=false,OpenCallback=bM,VisibleCallback=cy,Body=aU}local cz=n:GetTextSize(P,X,Enum.Font.SourceSans,Vector2.new(1e3,1e3))local cA=cz.x+Y*2;local cB=G.InvisibleButton(af.Frame.Interior.Tabs,'Tab_'..P)ar.ButtonFrame=cB;cB.Size=b(0,cA,0,u.TabHeight)cB.Visible=false;local cC=G.TextLabel(cB,'Label',P)cC.TextSize=X;af:ConnectButtonHover(cB,af:CreateButtonEffectFunction(cC))local cD=b(1,0,1,-7)local cE=b(0,0,0,4)local cF=b(1,0,1,-4)local cG=b(0,0,0,4)cC.Size=cD;cC.Position=cE;function ar.SetVisible(ar,ag)if ag==ar.Visible then return end;ar.Visible=ag;ar:SetOpen(false)if ar.VisibleCallback then ar.VisibleCallback(ag)end;cB.Visible=ag;af:RefreshTabs()if not ag then ar.SetOpen(false)end end;function ar.RecordInitialOpen(ar)end;function ar.RecordClickToOpen(ar)end;function ar.SetOpen(ar,b5)if b5==ar.Open then return end;ar.Open=b5;if b5 then if ar.SavedScrollbarValue then af.WindowScrollbar:SetValue(ar.SavedScrollbarValue)end;local cx=af.Tabs;for by=1,#cx do if cx[by]~=ar then cx[by]:SetOpen(false)end end;if aU then aU.Visible=true end;af:RefreshTabs()cC.Size=cF;cC.Position=cG;af.CurrentOpenedTab=cB else ar.SavedScrollbarValue=af.WindowScrollbar:GetValue()if aU then aU.Visible=false;af.Frame.Interior.WindowContainer.Window.Body.Size=b(1,0,1,0)af.Frame.Interior.WindowContainer.Window.Body.Position=b(0,0,0,0)end;cC.Size=cD;cC.Position=cE end;if ar.OpenCallback then ar.OpenCallback(b5)end end;cB.MouseButton1Click:connect(function()if ar.Visible then ar:RecordClickToOpen()ar:SetOpen(true)end end)table.insert(af.Tabs,ar)return ar end;function a7.ApplyScrollbarToFrame(af,aV,aT,aU,aw)local cH,cI;local bS=aV:GetHeight()local bn=aV:GetValue()local function cJ()return aT.AbsoluteSize.Y,aU.AbsoluteSize.Y end;local function cK()local cL,cM=cJ()if cI~=cM or cH~=cL then cI,cH=cM,cL;bS=cH/cI;aV:SetHeight(bS)local cN=(cI-cH)*bn;if cN<0 then cN=0 end;local ao=aU.Position.X;local ap=aU.Position.Y;aU.Position=b(ao.Scale,ao.Offset,ap.Scale,-math.floor(cN))end end;local function cO(cP)bn=cP;cK()local cN=(cI-cH)*bn;local ao=aU.Position.X;local ap=aU.Position.Y;aU.Position=b(ao.Scale,ao.Offset,ap.Scale,-math.floor(cN))end;aV.ValueChanged:connect(cO)cO(aV:GetValue())local cQ=120;aT.Active=true;aV.ButtonUp.MouseButton1Click:connect(function()aV:Scroll(-cQ,cJ())end)aV.ButtonDown.MouseButton1Click:connect(function()aV:Scroll(cQ,cJ())end)ai(aT,'AbsoluteSize',cK)ai(aU,'AbsoluteSize',function()local cH,cI=cJ()local bn=aV:GetValue()if bn~=1 and bn~=0 then local bn=-aU.Position.Y.Offset/(cI-cH)aV:SetValue(bn)end;cK()end)aT.MouseWheelForward:connect(function()aV:Scroll(-cQ,cJ())end)aT.MouseWheelBackward:connect(function()aV:Scroll(cQ,cJ())end)aT.TouchPan:connect(function(cR,cS,cT,cU)aV:Scroll(-cS.y,cJ())end)end;function a7.CreateScrollbar(af,cV)local aV={}local cW=nil;cW=G.FolderFrame(cW,'Scrollbar')aV.Frame=cW;local aw=G.Button(cW,'Frame')aw.AutoButtonColor=false;aw.Size=b(1,0,1,-u.HandleHeight*2-2)aw.Position=b(0,0,0,u.HandleHeight+1)local cX=G.Frame(cW,'Frame')cX.Size=b(1,0,1,0)cX.Position=b(0,0,0,0)function aV.SetVisible(aV,ag)aw.Visible=ag;cX.Visible=not ag end;local cY=G.ImageButton(aw,'Up','https://www.roblox.com/asset/?id=261880783')aV.ButtonUp=cY;cY.Size=b(1,0,0,u.HandleHeight)cY.Position=b(0,0,0,-u.HandleHeight-1)cY.AutoButtonColor=false;af:ConnectButtonHover(cY,af:CreateButtonEffectFunction(cY))local cZ=G.ImageButton(aw,'Down','https://www.roblox.com/asset/?id=261880783')aV.ButtonDown=cZ;cZ.Size=b(1,0,0,u.HandleHeight)cZ.Position=b(0,0,1,1)cZ.Rotation=180;cZ.AutoButtonColor=false;af:ConnectButtonHover(cZ,af:CreateButtonEffectFunction(cZ))local c_=G.Button(aw,'Bar')c_.Size=b(1,0,0.5,0)c_.Position=b(0,0,0.25,0)c_.AutoButtonColor=false;local d0=G.InvisibleImageLabel(c_,'Image','https://www.roblox.com/asset/?id=261904959')d0.Size=b(0,16,0,16)d0.Position=b(0.5,-8,0.5,-8)local aH=af:CreateButtonEffectFunction(c_,nil,c_.BackgroundColor3,c_.BackgroundColor3)local bn=1;local d1=Z()aV.ValueChanged=d1;local bS=0.25;local ch=Z()aV.HeightChanged=ch;local function d2(d3)return(d3-cW.AbsolutePosition.Y)/cW.AbsoluteSize.Y/(1-bS)end;local function d4()local ap=bS;c_.Size=b(1,0,ap,0)c_.Position=b(0,0,bn*(1-ap),0)end;d4()function aV.SetValue(aV,cP)if cP<0 then cP=0 elseif cP>1 then cP=1 end;if cP~=bn then bn=cP;d4()d1:fire(cP)end end;function aV.GetValue(aV)return bn end;function aV.Scroll(aV,ca,cH,cI)aV:SetValue(bn+ca/cI)end;function aV.SetHeight(aV,d5)if d5<0 then d5=0 elseif d5>1 then d5=1 end;d5=math.max(d5,0.1)if d5~=bS then bS=d5;aV:SetVisible(d5<1)d4()ch:fire(d5)end end;function aV.GetHeight(aV)return bS end;af:ConnectButtonDragging(c_,function()local d6=bn;return function(aP,aQ)local d7=aQ;local d8=d7/aw.AbsoluteSize.Y/(1-bS)aV:SetValue(d6+d8)end end,aH)return aV end;local d9;do local da=math.cos;local db=math.pi;function d9(dc,dd,ak)dc:fire()local de=tick()local aa=ac.RenderStepped:connect(function()local a9=(tick()-de)/dd;if a9>=1 then a9=1;dc:fire()else a9=(1-da(a9*db))/2 end;ak(a9)end)dc:connect(aa)return aa end end;if V then function a7.CreateButtonEffectFunction(af,aE,C,df,dg)C=C or aE.BackgroundColor3;df=df or u.GetButtonDownColor(C)dg=dg or u.GetButtonHoverColor(C)local dc=a6()return function(aI,aJ)local dh=aE.BackgroundColor3;local di=aI and df or(aJ and dg or C)local dj,dk,dl=dh.r,dh.g,dh.b;local dm,dn,dp=di.r,di.g,di.b;local dq,dr,ds=dm-dj,dn-dk,dp-dl;d9(dc,aI and 0.125 or 0.25,function(a9)aE.BackgroundColor3=c(dj+dq*a9,dk+dr*a9,dl+ds*a9)end)end end else function a7.CreateButtonEffectFunction(af,aE,C,df,dg)C=C or aE.BackgroundColor3;df=df or u.GetButtonDownColor(C)dg=dg or u.GetButtonHoverColor(C)return function(aI,aJ)aE.BackgroundColor3=aI and df or(aJ and dg or C)end end end;function a7.GenerateOptionButtonAnimationToggle(af,dt,aE,b3,aX,aZ,a_)local du=aX.Size;local dv=b(du.X.Scale,du.X.Offset+u.GearSize+2+u.BorderSize,du.Y.Scale,du.Y.Offset)local dw=b3.Rotation;local dx=b3.Rotation-90;local dy=dt.Size;local dz=b(dy.X.Scale,dy.X.Offset,dy.Y.Scale,dy.Y.Offset-u.OptionAreaHeight)local dA=dt.Position;local dB=b(dA.X.Scale,dA.X.Offset,dA.Y.Scale,dA.Y.Offset+u.OptionAreaHeight)local dd=0.5;local bQ=a6()return function(b5)if b5 then dt:TweenSizeAndPosition(dz,dB,'Out','Sine',dd,true)aX:TweenSize(dv,'Out','Sine',dd,true)aZ:TweenSizeAndPosition(b(1,0,0,u.OptionAreaHeight),b(0,0,0,-u.OptionAreaHeight),'Out','Sine',dd,true)a_:TweenPosition(b(0,0,0,0),'Out','Sine',dd,true)local dC=b3.Rotation;d9(bQ,dd,function(a9)b3.Rotation=dx*a9+dC*(1-a9)end)else dt:TweenSizeAndPosition(dy,dA,'Out','Sine',dd,true)aX:TweenSize(du,'Out','Sine',dd,true)aZ:TweenSizeAndPosition(b(1,0,0,0),b(0,0,0,0),'Out','Sine',dd,true)a_:TweenPosition(b(0,0,0,u.OptionAreaHeight),'Out','Sine',dd,true)local dC=b3.Rotation;d9(bQ,dd,function(a9)b3.Rotation=dw*a9+dC*(1-a9)end)end end end;do local dD=Z()function a7.ConnectButtonHover(af,aE,dE)local _={}local aI=false;local aJ=false;local function dF(dG,dH)if dH and ad then dH=false end;if dG~=aI or dH~=aJ then aI,aJ=dG,dH;dE(aI,aJ)end end;aE.MouseButton1Down:connect(function()dF(true,true)end)aE.MouseButton1Up:connect(function()dF(false,true)end)aE.MouseEnter:connect(function()dF(aI,true)end)aE.MouseLeave:connect(function()dF(false,false)end)end end;function a7.ConnectButtonDragging(af,aE,dI,dE)local aM=false;local aJ=false;local dJ={}local dK;local function dL()if not aM then return end;aM=false;dE(aM,aJ)for by=#dJ,1,-1 do dJ[by]:disconnect()dJ[by]=nil end end;local dM={[Enum.UserInputType.MouseButton1]=true,[Enum.UserInputType.Touch]=true}local bV=game:GetService("Players").LocalPlayer:GetMouse()local function dN(dO)if aM then return end;aM=true;dE(aM,aJ)local dP;local aN,aO=dO.X,dO.Y;dJ[#dJ+1]=ab.InputEnded:connect(function(bY)if dM[bY.UserInputType]then dL()end end)dJ[#dJ+1]=ab.InputChanged:connect(function(bY)if not(bY.UserInputType==Enum.UserInputType.MouseMovement or bY.UserInputType==Enum.UserInputType.Touch)then return end;local dQ=bY.Position;if not dQ then return end;local dR,dS=dQ.X,dQ.Y;if not dP then dP,dK=dI(aN or dR,aO or dS)end;if aN then dP(dR-aN,dS-aO)end end)end;aE.InputBegan:connect(function(dT)if dT.UserInputType==Enum.UserInputType.Touch or dT.UserInputType==Enum.UserInputType.MouseButton1 then dN(dT.Position)end end)aE.InputEnded:connect(function(dT)if dT.UserInputType==Enum.UserInputType.Touch or dT.UserInputType==Enum.UserInputType.MouseButton1 then dL()end end)aE.MouseEnter:connect(function()if not aJ then aJ=true;dE(aM,aJ)end end)aE.MouseLeave:connect(function()if aJ then aJ=false;dE(aM,aJ)end end)af.VisibleChanged:connect(dL)end;do local dU,at=false;function ae.GetPermissions()while dU do wait()end;if at then return at end;at={}dU=true;at.IsCreator=false;local dV,dW=pcall(function()local dX=string.format("/users/%d/canmanage/%d",game:GetService("Players").LocalPlayer.UserId,game.PlaceId)return game:GetService('HttpRbxApiService'):GetAsync(dX,Enum.ThrottlingPriority.Default,Enum.HttpRequestType.Default,true)end)if dV and type(dW)=="string"then dV,dW=pcall(function()return game:GetService('HttpService'):JSONDecode(dW)end)if dV and dW.CanManage==true then at.IsCreator=dW.CanManage end end;at.ClientCodeExecutionEnabled=false;pcall(function()at.ServerCodeExecutionEnabled=at.IsCreator and not settings():GetFFlag("DebugDisableLogServiceExecuteScript")end)if f or ac:IsStudio()then at.IsCreator=true;at.ServerCodeExecutionEnabled=true end;at.MayViewServerLog=at.IsCreator;at.MayViewClientLog=true;at.MayViewServerStats=at.IsCreator;at.MayViewServerMemory=at.IsCreator;at.MayViewServerScripts=at.IsCreator;at.MayViewServerJobs=at.IsCreator;at.MayViewDataStoreBudget=false;pcall(function()at.MayViewDataStoreBudget=at.IsCreator end)at.MayViewHttpResultClient=false;at.MayViewHttpResultClient=at.IsCreator;at.MayViewHttpResultServer=false;at.MayViewHttpResultServer=at.IsCreator;at.MayViewContextActionBindings=at.IsCreator;dU=false;return at end end;do local au;function ae.GetMessagesAndStats(at)if au then return au end;local function dY(dZ)local _;_={Messages=nil,MessageAdded=Z(),GetMessages=function()local bR=_.Messages;if not bR then if _.Attempted then bR={}else _.Attempted=true;bR=dZ(_)_.Messages=bR end end;return bR end}return _ end;local d_;do local function e0(e1)return(e1<10 and"0"or"")..e1 end;local e2=string.format;function d_(de)local e3=de-e()+math.floor(tick())local e4=e3%86400;local e5=math.floor(e4/3600)e4=e4-e5*3600;local e6=math.floor(e4/60)e4=e4-e6*60;local e7=e4;local e8=e0(e5)local e9=e0(e6)local ea=e0(e4)return e2("%s:%s:%s",e8,e9,ea)end end;local eb={"ClassDescriptor failed to learn","EventDescriptor failed to learn","Type failed to learn"}local function ec(br)if br.Type~=Enum.MessageType.MessageWarning.Value then return false end;local ed=false;for a5,ee in ipairs(eb)do if string.find(br.Message,ee)~=nil then ed=true;break end end;return ed end;local ef;if at.MayViewClientLog then ef=dY(function(_)local bR={}local eg=game:GetService("LogService")do local eh=eg:GetLogHistory()for by=1,#eh do local ei=eh[by]local br={Message=ei.message or"[DevConsole Error 1]",Time=d_(ei.timestamp),Type=ei.messageType.Value}if not ec(br)then bR[#bR+1]=br end end end;eg.MessageOut:connect(function(P,ej)local br={Message=P or"[DevConsole Error 2]",Time=d_(e()),Type=ej.Value}if not ec(br)then bR[#bR+1]=br;_.MessageAdded:fire(br)end end)return bR end)end;local ek;if at.MayViewServerLog then ek=dY(function(_)local bR={}local eg=game:GetService("LogService")eg.ServerMessageOut:connect(function(P,ej,el)local br={Message=P or"[DevConsole Error 3]",Time=d_(el),Type=ej.Value}if not ec(br)then bR[#bR+1]=br;_.MessageAdded:fire(br)end end)eg:RequestServerOutput()return bR end)end;local em;if at.MayViewServerStats or at.MayViewServerScripts or at.MayViewServerMemory then em={Stats=nil,StatsReceived=Z()}local en;function em.GetStats(em)local bD=em.Stats;if not bD then bD={}pcall(function()local eo=game:FindService("NetworkClient"):GetChildren()[1]if eo then en=eo.StatsReceived:connect(function(bE)em.StatsReceived:fire(bE)end)eo:RequestServerStats(true)end end)em.Stats=bD end;return bD end end;au={OutputMessageSyncLocal=ef,OutputMessageSyncServer=ek,StatsSyncServer=em}return au end end;local ep={}local eq=nil;local function er(es)local et="blockMenuAction"local eu="closeDevConsoleAction"local ev="devConsoleSelectionGroup"local function ew(ex,ey,ez)if ey==Enum.UserInputState.End then eq:SetVisible(false)end end;if es then m:BindCoreAction(et,function()end,false,Enum.KeyCode.ButtonStart)local eA=require(l.Settings.SettingsHub)eA:SetVisibility(false,true)m:BindCoreAction(eu,ew,false,Enum.KeyCode.ButtonB)o:AddSelectionParent(ev,eq.Frame)o.SelectedCoreObject=eq.CurrentOpenedTab else m:UnbindCoreAction(eu)m:UnbindCoreAction(et)o:RemoveSelectionGroup(ev)o.SelectedCoreObject=nil end end;local eB=false;local function eC()if not eq and not eB then eB=true;local at=ae.GetPermissions()local au=ae.GetMessagesAndStats(at)eq=ae.new(k,at,au)if q then eq.VisibleChanged:connect(er)end;eB=false end;return eq end;function ep:GetVisibility()local af=eC()if af then return af.Visible else return false end end;function ep:SetVisibility(bn)local af=eC()if af then af:SetVisible(bn)end end;local eD=false;local eE=false;local function eF()if eD then return end;eD=true;spawn(function()ep:GetVisibility()ep:SetVisibility(eE)eD=false end)end;local eG=game:GetService("StarterGui")local function eH()if not eq then eF()return eE else return ep:GetVisibility()end end;local function eI(ag)if type(ag)~="boolean"then error("DeveloperConsoleVisible must be given a boolean value.")end;if not eq then eE=ag;eF()else ep:SetVisibility(ag)end end;local eJ=game:GetService'UserInputService'local eG=game:GetService'StarterGui'eJ.InputBegan:connect(function(eK)if eK.UserInputType==Enum.UserInputType.Keyboard and eK.KeyCode==Enum.KeyCode.F9 then local F=eH()eG:SetCore('DevConsoleVisible',false)eI(not F)end end)]==])
